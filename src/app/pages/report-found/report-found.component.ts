...
@Component({
    ...
    styleUrl: './report-found.component.scss'
})
export class ReportFoundComponent {
    ...
    onSubmit(): void {
        if (this.foundForm.valid) {
            const formData = this.foundForm.value;
            this.itemService.reportFoundItem(formData);

            this.snackBar.open('Bulunan eşya bildirimi başarıyla oluşturuldu!', 'Tamam', {
                duration: 3000,
    ...
    private markFormGroupTouched(): void {
        Object.keys(this.foundForm.controls).forEach(key => {
            this.foundForm.get(key)?.markAsTouched();
        });
    }

    getErrorMessage(controlName: string): string {
        const control = this.foundForm.get(controlName);
        if (!control) return '';
